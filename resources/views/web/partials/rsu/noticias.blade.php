@php
    ...
    $newsItems = collect($noticias ?? [])->take(3)->map(function ($noticia) {
        $fecha = \Carbon\Carbon::parse($noticia->fecha)->locale('es');

        return [
            'src' => $noticia->imagen
                ? asset('noticias_imagenes/' . $noticia->imagen)
                : asset('web/imagenes/bienestar/rsu/noticia-1.jpg'),
            'date' => $fecha->translatedFormat('d') . ' ' . ucfirst($fecha->translatedFormat('F')) . ', ' . $fecha->format('Y'),
...
<section class="rsu-news" id="noticias">
    <div class="container">
        ...
        <div class="rsu-news__grid">
            ...
            <article class="rsu-card rsu-news-card">
                <img
                    src="{{ $item['src'] }}"
                    alt="{{ $item['title'] }}"
